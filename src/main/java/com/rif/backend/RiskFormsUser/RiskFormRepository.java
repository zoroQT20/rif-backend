...
public interface RiskFormRepository extends JpaRepository<RiskFormData, Long> {
    ...
    @Query(value = "SELECT rf.sda_number as sdaNumber, rf.issue_particulars as issueParticulars, " +
                   "GROUP_CONCAT(rp.description) as riskParticularDescriptions, p.unit as unit, rf.submission_date as submissionDate, rf.risk_rating as riskRating " +
                   "FROM risk_forms rf " +
                   ...
                   "GROUP BY rf.sda_number, rf.issue_particulars, p.unit, rf.submission_date, rf.risk_rating", nativeQuery = true)

...
public class RiskIdentificationFormController {
    ...
    public ResponseEntity<?> createRiskForm(@Valid @RequestBody RiskIdentificationFormEntity riskIdentificationForm, BindingResult bindingResult) {
        ...
        try {
            ...
            logger.error("Data integrity violation occurred while saving risk form: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to create risk form due to data integrity violation");
        } catch (Exception e) {
            // Log and return error response
